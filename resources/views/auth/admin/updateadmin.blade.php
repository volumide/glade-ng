...
@section('content')
    <form action="/admin/register" method="POST" class="pb-10 shadow-md mt-5 rounded overflow-hidden lg:w-2/5 mx-auto">
        @csrf
        <h1 class="text-2xl text-center mb-10 bg-gray-800 py-5  text-white font-bold">Admin Registeration </h1>
        ...
        <div class="w-full px-4">
            <div class="mb-5">
                ...
                <input type="email" placeholder="Email" name="email"
                    value="{{ @old('email') ? @old('name') : $data->name }}"
                    class=" w-full border-[1.5px] border-form-stroke rounded-lg py-3 px-5 font-medium text-body-color placeholder-body-color outline-none focus:border-primary active:border-primary transition disabled:bg-[#F5F7FD] disabled:cursor-default">
                @error('email')
                    <p class="text-red-400">{{ $message }}</p>
                @enderror
